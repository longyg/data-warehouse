...
#
# Usage:
#      ods_to_dwd_db.sh <first|all> <date>
# Example:
#      ods_to_dwd_db.sh first 2020-10-01
#
###############################################################################
...
select
	...
	sku.create_time
from
	(select * from ${APP}.ods_sku_info where dt='$do_date') sku
join
	(select * from ${APP}.ods_base_trademark where dt='$do_date') ob
	on sku.tm_id = ob.tm_id
join
	(select * from ${APP}.ods_spu_info where dt='$do_date') spu
	on spu.id=sku.spu_id
join
	(select * from ${APP}.ods_base_category3 where dt='$do_date') c3
	on sku.category3_id = c3.id
join
	(select * from ${APP}.ods_base_category2 where dt='$do_date') c2
	on c3.category2_id = c2.id
join
	(select * from ${APP}.ods_base_category1 where dt='$do_date') c1
...
join
	(select * from ${APP}.ods_order_info where dt='$do_date') oi
on 
	oi.id = pi.order_id;
...
select 
	...
	source_id,
	original_amount_d,
	if(rn=1,final_total_amount-sum_final_amount_d+final_amount_d,final_amount_d),
	if(rn=1,feight_fee-sum_feight_fee_d+feight_fee_d,feight_fee_d),
	if(rn=1,benefit_reduce_amount-sum_benefit_reduce_amount_d+benefit_reduce_amount_d,benefit_reduce_amount_d)
from 
(
	select
		od.id,
		...
		sum(round((od.order_price*od.sku_num/oi.original_total_amount)*oi.final_total_amount,2)) over (partition by od.order_id) sum_final_amount_d,
		sum(round((od.order_price*od.sku_num/oi.original_total_amount)*oi.feight_fee, 2)) over (partition by od.order_id) sum_feight_fee_d,
		sum(round((od.order_price*od.sku_num/oi.original_total_amount)*oi.benefit_reduce_amount, 2)) over (partition by od.order_id) sum_benefit_reduce_amount_d,
		row_number() over (partition by od.order_id order by od.id desc) rn
	from
		(select * from ${APP}.ods_order_detail where dt='$do_date') od
	join
		(select * from ${APP}.ods_order_info where dt='$do_date') oi
	on
		od.order_id = oi.id
...
from 
(
	select * from ${APP}.dwd_fact_order_info 
	where dt in
	(
		select 
...
select 
	...
	br.region_name 
from 
	(select * from ${APP}.ods_base_province ) bp 
join 
	(select * from ${APP}.ods_base_region ) br 
on 
	bp.region_id = br.id;
"
